// Dead Space 3 Linear Mouse Fix - Experimental A/B v0.8.1
// x86 / no CRT / selective mouse-look validation build.
//
// Keeps the proven v0.8 selective AB9830 bypass, then calls D820 vanilla first
// so its history/state updates remain untouched. Only on the verified mouse-look
// camera path and only while a per-thread mouse-tail latch is active, the hook
...
static MouseTailSlotV81* mouse_tail_slot(UINT32 tid,BOOL create){
    ...
    if(!create)return 0;
    for(UINT32 i=0;i<32;++i){LONG expected=0;if(__sync_bool_compare_and_swap(&g_mouse_tail[i].thread_id,expected,(LONG)tid)){g_mouse_tail[i].active=0;g_mouse_tail[i].generation=0;g_mouse_tail[i].seen_generation=0;g_mouse_tail[i].stable_under_count=0;g_mouse_tail[i].last_bypass_qpc=0;return &g_mouse_tail[i];}}
    return 0;
}
static void mouse_tail_start(UINT32 tid,long long now){
    MouseTailSlotV81* s=mouse_tail_slot(tid,TRUE);if(!s)return;
    s->last_bypass_qpc=now;s->stable_under_count=0;s->active=1;__sync_add_and_fetch(&s->generation,1);__sync_add_and_fetch(&g_tail_starts,1);
}
...
extern "C" void __attribute__((fastcall)) hook_ab9830(void* self,void*,float* x,float* y,float curve0,float curve1){
    ...
    if(targeted){
        ...
        if(g_bypass_enabled&&fresh_nonzero&&pre_over&&x&&y){*x=pre_x;*y=pre_y;r.p.ab9830.flags|=16U;__sync_add_and_fetch(&g_bypass_applied,1);mouse_tail_start(tid,qpc_now());}
...
// -----------------------------------------------------------------------------
// D820: vanilla first, then selective counterfactual unclamp on the verified mouse-look path.
// -----------------------------------------------------------------------------
extern "C" BOOL __attribute__((fastcall)) hook_processor(void* self,void*,float dt,float* x,float* y){
    ...
    UINT32 tid=r.c.thread_id;UINT32 ret_addr=(UINT32)(ULONG_PTR)ret;BOOL target=(g_base&&ret_addr==(UINT32)(ULONG_PTR)(g_base+kTargetD820ReturnRva));if(target)r.p.processor.correction_flags|=2U;
    ...
    if(g_bypass_enabled&&target&&tail&&tail->active){
        ...
        if(timed_out){tail->active=0;tail->stable_under_count=0;__sync_add_and_fetch(&g_tail_timeouts,1);}
        else if(model_ok){
            ...
            if(over&&x&&y){BOOL tail_only=(tail->seen_generation==gen);float corrected_x=model.preclamp_x*model.scale,corrected_y=model.preclamp_y*model.scale;*x=corrected_x;*y=corrected_y;r.p.processor.correction_flags|=16U;if(tail_only){r.p.processor.correction_flags|=32U;__sync_add_and_fetch(&g_d820_tail_applied,1);}tail->seen_generation=gen;tail->stable_under_count=0;__sync_add_and_fetch(&g_d820_unclamp_applied,1);}
            else{LONG stable=__sync_add_and_fetch(&tail->stable_under_count,1);if(stable>=(LONG)kTailStableSamples){tail->active=0;tail->stable_under_count=0;__sync_add_and_fetch(&g_tail_stable_exits,1);}}
...
static DWORD WINAPI init_thread(LPVOID){
    ...
    LogHeaderV6 h;zero_bytes(&h,sizeof(h));h.magic[0]='D';h.magic[1]='S';h.magic[2]='3';h.magic[3]='L';h.magic[4]='M';h.magic[5]='8';h.magic[6]='1';h.version=81;h.header_size=sizeof(h);h.record_size=sizeof(LogRecordV6);h.flags=31;h.qpc_frequency=fq.QuadPart;h.image_timestamp=ts;h.image_size=image_size;h.text_rva=text_rva;h.queue_capacity_records=kQueueCapacity;h.flush_interval_ms=kFlushIntervalMs;h.module_rescan_ms=kModuleRescanMs;h.rawreg_poll_ms=kRawRegPollMs;DWORD wrote=0;if(!WriteFile(g_log,&h,sizeof(h),&wrote,0)||wrote!=sizeof(h)){status_text("REFUSED: failed to write binary header.\r\n");return 0;}
