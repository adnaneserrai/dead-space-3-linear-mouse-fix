...
def main():
    ...
    if magic!=b'DS3LM81\0' or version!=81 or header_size!=HEADER.size or record_size!=RECORD_SIZE:raise SystemExit(f'unsupported header magic={magic!r} version={version}')
    ...
    print('DS3 Linear Mouse Fix - Experimental A/B v0.8.1')
    ...
    for r in records:
        ...
        if typ==8:
            ...
            ab_rows.append({'time_s':t,'qpc_begin':r['qpc_begin'],'qpc_end':r['qpc_end'],'caller':caller,'thread':r['thread'],'pre_x':u[10],'pre_y':u[11],'pre_magnitude':math.hypot(u[10],u[11]),'vanilla_post_x':u[12],'vanilla_post_y':u[13],'effective_post_x':u[14],'effective_post_y':u[15],'fresh_di_x':u[16],'fresh_di_y':u[17],'bypass_applied':int(bool(fl&16))})
        elif typ==11:
