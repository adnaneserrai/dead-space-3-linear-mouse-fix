...
//
// Observes the game's existing DirectInput8 mouse path and the already verified
// D820 processor. No input values are changed. No Raw Input device is registered.
// No WndProc is subclassed. deadspace3.exe is never modified on disk.
...
// -----------------------------------------------------------------------------
// Binary log format v6.0. Every record is 512 bytes.
// -----------------------------------------------------------------------------
enum RecordTypeV6 {
    REC_MODULE=1,
    REC_DI8_CREATE=2,
...
#pragma pack(push,1)
struct LogHeaderV6 {
    BYTE magic[8];              // "DS3DI60\0"
    UINT32 version;             // 60
    UINT32 header_size;
    UINT32 record_size;
    UINT32 flags;               // bit0 buffered writer, bit1 DI observer, bit2 passive raw-reg monitor
...
struct Di8CreatePayloadV6 {
    UINT32 call_id,hinst,version,riid_ptr,out_ptr,punk_outer,result_hr,last_error,object_ptr,wrapped;
    GUID_X requested_iid; UINT32 reserved[12];
};
struct CreateDevicePayloadV6 {
    UINT32 call_id,di8_ptr,guid_ptr,out_ptr,punk_outer,result_hr,last_error,device_ptr,is_mouse_guid,wrapped;
...
struct MetaPayloadV6 { UINT32 processor_rva,signature_matches,callsite_count,d820_hook_installed,di_iat_patched,raw_monitor_active,reserved[10]; };
...
union PayloadV6 {
    ...
    MetaPayloadV6 meta; IatPayloadV6 iat; BYTE pad[472];
};
...
static volatile LONG g_status_di_confirmed=0,g_status_processor_confirmed=0,g_status_drop_reported=0;
...
struct DI8Wrap {
    LPVOID object; LPVOID* original_vtable; LPVOID* shadow_vtable; DI8CreateDeviceFn original_create_device;
};
struct MouseWrap {
    LPVOID object; LPVOID* original_vtable; LPVOID* shadow_vtable;
    DIDSetDataFormatFn original_setformat; DIDSetCooperativeLevelFn original_setcoop; DIDAcquireFn original_acquire;
    DIDUnacquireFn original_unacquire; DIDGetDeviceStateFn original_getstate; DIDGetDeviceDataFn original_getdata;
    UINT32 format_kind,x_offset,y_offset,z_offset,data_size;
};
static DI8Wrap g_di8_wraps[8]; static UINT32 g_di8_wrap_count=0;
static MouseWrap g_mouse_wraps[8]; static UINT32 g_mouse_wrap_count=0;
...
static void count_drop(UINT32 type){__sync_add_and_fetch(&g_dropped_total,1);if(type==REC_PROCESSOR)__sync_add_and_fetch(&g_dropped_processor,1);else __sync_add_and_fetch(&g_dropped_di,1);}
...
// -----------------------------------------------------------------------------
// COM shadow-vtable helpers
// -----------------------------------------------------------------------------
static DI8Wrap* find_di8_wrap(LPVOID object){for(UINT32 i=0;i<g_di8_wrap_count;++i)if(g_di8_wraps[i].object==object)return&g_di8_wraps[i];return 0;}
static MouseWrap* find_mouse_wrap(LPVOID object){for(UINT32 i=0;i<g_mouse_wrap_count;++i)if(g_mouse_wraps[i].object==object)return&g_mouse_wraps[i];return 0;}
...
static BOOL wrap_di8(LPVOID object){if(!object||!readable_span(object,sizeof(LPVOID)))return FALSE;tiny_lock(&g_wrap_lock);if(find_di8_wrap(object)){tiny_unlock(&g_wrap_lock);return TRUE;}if(g_di8_wrap_count>=8){tiny_unlock(&g_wrap_lock);return FALSE;}LPVOID* vt=*(LPVOID**)object;if(!vt||!readable_span(vt,11*sizeof(LPVOID))){tiny_unlock(&g_wrap_lock);return FALSE;}LPVOID* shadow=(LPVOID*)pVirtualAlloc(0,11*sizeof(LPVOID),MEM_COMMIT|MEM_RESERVE,PAGE_READWRITE);if(!shadow){tiny_unlock(&g_wrap_lock);return FALSE;}for(UINT32 i=0;i<11;++i)shadow[i]=vt[i];UINT32 idx=g_di8_wrap_count;DI8Wrap* w=&g_di8_wraps[idx];w->object=object;w->original_vtable=vt;w->shadow_vtable=shadow;w->original_create_device=(DI8CreateDeviceFn)vt[3];shadow[3]=(LPVOID)hook_DI8_CreateDevice;__sync_synchronize();g_di8_wrap_count=idx+1;*(LPVOID**)object=shadow;tiny_unlock(&g_wrap_lock);return TRUE;}
static BOOL wrap_mouse(LPVOID object){if(!object||!readable_span(object,sizeof(LPVOID)))return FALSE;tiny_lock(&g_wrap_lock);if(find_mouse_wrap(object)){tiny_unlock(&g_wrap_lock);return TRUE;}if(g_mouse_wrap_count>=8){tiny_unlock(&g_wrap_lock);return FALSE;}LPVOID* vt=*(LPVOID**)object;if(!vt||!readable_span(vt,32*sizeof(LPVOID))){tiny_unlock(&g_wrap_lock);return FALSE;}LPVOID* shadow=(LPVOID*)pVirtualAlloc(0,32*sizeof(LPVOID),MEM_COMMIT|MEM_RESERVE,PAGE_READWRITE);if(!shadow){tiny_unlock(&g_wrap_lock);return FALSE;}for(UINT32 i=0;i<32;++i)shadow[i]=vt[i];UINT32 idx=g_mouse_wrap_count;MouseWrap* w=&g_mouse_wraps[idx];zero_bytes(w,sizeof(*w));w->object=object;w->original_vtable=vt;w->shadow_vtable=shadow;w->original_acquire=(DIDAcquireFn)vt[7];w->original_unacquire=(DIDUnacquireFn)vt[8];w->original_getstate=(DIDGetDeviceStateFn)vt[9];w->original_getdata=(DIDGetDeviceDataFn)vt[10];w->original_setformat=(DIDSetDataFormatFn)vt[11];w->original_setcoop=(DIDSetCooperativeLevelFn)vt[13];w->x_offset=w->y_offset=w->z_offset=UINT_ERROR;shadow[7]=(LPVOID)hook_Mouse_Acquire;shadow[8]=(LPVOID)hook_Mouse_Unacquire;shadow[9]=(LPVOID)hook_Mouse_GetDeviceState;shadow[10]=(LPVOID)hook_Mouse_GetDeviceData;shadow[11]=(LPVOID)hook_Mouse_SetDataFormat;shadow[13]=(LPVOID)hook_Mouse_SetCooperativeLevel;__sync_synchronize();g_mouse_wrap_count=idx+1;*(LPVOID**)object=shadow;tiny_unlock(&g_wrap_lock);__sync_add_and_fetch(&g_mouse_devices,1);return TRUE;}

// -----------------------------------------------------------------------------
// DirectInput hooks
// -----------------------------------------------------------------------------
extern "C" HRESULT WINAPI hook_DirectInput8Create(HINSTANCE hinst,DWORD version,const GUID_X* riid,LPVOID* out,LPVOID punk){
    void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();LogRecordV6 r;common_fill(&r,REC_DI8_CREATE,ret);r.p.di8.call_id=(UINT32)__sync_add_and_fetch(&g_call_id,1);r.p.di8.hinst=(UINT32)(ULONG_PTR)hinst;r.p.di8.version=version;r.p.di8.riid_ptr=(UINT32)(ULONG_PTR)riid;r.p.di8.out_ptr=(UINT32)(ULONG_PTR)out;r.p.di8.punk_outer=(UINT32)(ULONG_PTR)punk;if(riid&&readable_span(riid,sizeof(GUID_X)))r.p.di8.requested_iid=*riid;pSetLastError(incoming_error);HRESULT hr=g_real_DirectInput8Create(hinst,version,riid,out,punk);DWORD api_error=pGetLastError();r.p.di8.result_hr=(UINT32)hr;r.p.di8.last_error=api_error;LPVOID object=(hr_success(hr)&&out&&readable_span(out,sizeof(LPVOID)))?*out:0;r.p.di8.object_ptr=(UINT32)(ULONG_PTR)object;r.p.di8.wrapped=object&&wrap_di8(object)?1U:0U;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_di8create_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;
}

extern "C" HRESULT WINAPI hook_DI8_CreateDevice(LPVOID self,const GUID_X* rguid,LPVOID* out,LPVOID punk){
    void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();DI8Wrap* w=find_di8_wrap(self);if(!w||!w->original_create_device){pSetLastError(incoming_error);return (HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_CREATE_DEVICE,ret);r.p.create_device.call_id=(UINT32)__sync_add_and_fetch(&g_call_id,1);r.p.create_device.di8_ptr=(UINT32)(ULONG_PTR)self;r.p.create_device.guid_ptr=(UINT32)(ULONG_PTR)rguid;r.p.create_device.out_ptr=(UINT32)(ULONG_PTR)out;r.p.create_device.punk_outer=(UINT32)(ULONG_PTR)punk;BOOL mouse=FALSE;if(rguid&&readable_span(rguid,sizeof(GUID_X))){r.p.create_device.device_guid=*rguid;mouse=is_mouse_guid(rguid);}r.p.create_device.is_mouse_guid=mouse?1U:0U;pSetLastError(incoming_error);HRESULT hr=w->original_create_device(self,rguid,out,punk);DWORD api_error=pGetLastError();LPVOID dev=(hr_success(hr)&&out&&readable_span(out,sizeof(LPVOID)))?*out:0;r.p.create_device.result_hr=(UINT32)hr;r.p.create_device.last_error=api_error;r.p.create_device.device_ptr=(UINT32)(ULONG_PTR)dev;r.p.create_device.wrapped=(mouse&&dev&&wrap_mouse(dev))?1U:0U;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_create_device_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;
}

extern "C" HRESULT WINAPI hook_Mouse_SetDataFormat(LPVOID self,const DIDATAFORMAT_X* fmt){
    void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_setformat){pSetLastError(incoming_error);return (HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_SET_DATA_FORMAT,ret);r.p.setformat.device_ptr=(UINT32)(ULONG_PTR)self;r.p.setformat.format_ptr=(UINT32)(ULONG_PTR)fmt;UINT32 x=UINT_ERROR,y=UINT_ERROR,z=UINT_ERROR,kind=0,captured=0;BOOL readable=fmt&&readable_span(fmt,sizeof(DIDATAFORMAT_X));if(readable){r.p.setformat.dw_size=fmt->dwSize;r.p.setformat.dw_obj_size=fmt->dwObjSize;r.p.setformat.dw_flags=fmt->dwFlags;r.p.setformat.dw_data_size=fmt->dwDataSize;r.p.setformat.dw_num_objs=fmt->dwNumObjs;r.p.setformat.format_readable=1;if(fmt->rgodf&&fmt->dwObjSize>=sizeof(DIOBJECTDATAFORMAT_X)&&fmt->dwNumObjs<=128&&readable_span(fmt->rgodf,(SIZE_T)fmt->dwObjSize*fmt->dwNumObjs)){for(DWORD i=0;i<fmt->dwNumObjs;++i){const DIOBJECTDATAFORMAT_X* o=(const DIOBJECTDATAFORMAT_X*)((const BYTE*)fmt->rgodf+(SIZE_T)i*fmt->dwObjSize);UINT32 gk=0;if(o->pguid&&readable_span(o->pguid,sizeof(GUID_X)))gk=guid_kind(o->pguid);if(gk==1)x=o->dwOfs;else if(gk==2)y=o->dwOfs;else if(gk==3)z=o->dwOfs;if(captured<16){r.p.setformat.objects[captured].guid_kind=gk;r.p.setformat.objects[captured].offset=o->dwOfs;r.p.setformat.objects[captured].type=o->dwType;r.p.setformat.objects[captured].flags=o->dwFlags;++captured;}}}}
    ...
    r.p.setformat.axis_x_offset=x;r.p.setformat.axis_y_offset=y;r.p.setformat.axis_z_offset=z;r.p.setformat.format_kind=kind;r.p.setformat.captured_objects=captured;pSetLastError(incoming_error);HRESULT hr=w->original_setformat(self,fmt);DWORD api_error=pGetLastError();r.p.setformat.result_hr=(UINT32)hr;r.p.setformat.last_error=api_error;if(hr_success(hr)&&readable){w->format_kind=kind;w->x_offset=x;w->y_offset=y;w->z_offset=z;w->data_size=fmt->dwDataSize;}r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_setformat_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;
}
extern "C" HRESULT WINAPI hook_Mouse_SetCooperativeLevel(LPVOID self,HWND hwnd,DWORD flags){void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_setcoop){pSetLastError(incoming_error);return(HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_SET_COOP,ret);r.p.coop.device_ptr=(UINT32)(ULONG_PTR)self;r.p.coop.hwnd=(UINT32)(ULONG_PTR)hwnd;r.p.coop.flags=flags;pSetLastError(incoming_error);HRESULT hr=w->original_setcoop(self,hwnd,flags);DWORD api_error=pGetLastError();r.p.coop.result_hr=(UINT32)hr;r.p.coop.last_error=api_error;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_setcoop_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;}
extern "C" HRESULT WINAPI hook_Mouse_Acquire(LPVOID self){void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_acquire){pSetLastError(incoming_error);return(HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_ACQUIRE,ret);r.p.acquire.device_ptr=(UINT32)(ULONG_PTR)self;pSetLastError(incoming_error);HRESULT hr=w->original_acquire(self);DWORD api_error=pGetLastError();r.p.acquire.result_hr=(UINT32)hr;r.p.acquire.last_error=api_error;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_acquire_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;}
extern "C" HRESULT WINAPI hook_Mouse_Unacquire(LPVOID self){void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_unacquire){pSetLastError(incoming_error);return(HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_UNACQUIRE,ret);r.p.acquire.device_ptr=(UINT32)(ULONG_PTR)self;pSetLastError(incoming_error);HRESULT hr=w->original_unacquire(self);DWORD api_error=pGetLastError();r.p.acquire.result_hr=(UINT32)hr;r.p.acquire.last_error=api_error;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_unacquire_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;}
extern "C" HRESULT WINAPI hook_Mouse_GetDeviceState(LPVOID self,DWORD cb,LPVOID data){void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_getstate){pSetLastError(incoming_error);return(HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_GET_STATE,ret);r.p.state.device_ptr=(UINT32)(ULONG_PTR)self;r.p.state.cb_data=cb;r.p.state.data_ptr=(UINT32)(ULONG_PTR)data;r.p.state.format_kind=w->format_kind;r.p.state.x_offset=w->x_offset;r.p.state.y_offset=w->y_offset;r.p.state.z_offset=w->z_offset;pSetLastError(incoming_error);HRESULT hr=w->original_getstate(self,cb,data);DWORD api_error=pGetLastError();r.p.state.result_hr=(UINT32)hr;r.p.state.last_error=api_error;BOOL valid=FALSE;if(hr_success(hr)&&data&&w->x_offset!=UINT_ERROR&&w->y_offset!=UINT_ERROR&&w->x_offset+4<=cb&&w->y_offset+4<=cb&&readable_span(data,cb)){r.p.state.buffer_readable=1;r.p.state.x=*(const INT32*)((const BYTE*)data+w->x_offset);r.p.state.y=*(const INT32*)((const BYTE*)data+w->y_offset);if(w->z_offset!=UINT_ERROR&&w->z_offset+4<=cb)r.p.state.z=*(const INT32*)((const BYTE*)data+w->z_offset);valid=TRUE;r.p.state.valid_xyz=1;__sync_add_and_fetch(&g_getstate_mouse_packets,1);if(r.p.state.x||r.p.state.y){__sync_add_and_fetch(&g_getstate_nonzero,1);}}(void)valid;r.c.qpc_end=qpc_now();__sync_add_and_fetch(&g_getstate_calls,1);enqueue_record(&r);pSetLastError(api_error);return hr;}
extern "C" HRESULT WINAPI hook_Mouse_GetDeviceData(LPVOID self,DWORD cbobj,DIDEVICEOBJECTDATA_X* data,DWORD* count,DWORD flags){void* ret=__builtin_return_address(0);DWORD incoming_error=pGetLastError();MouseWrap* w=find_mouse_wrap(self);if(!w||!w->original_getdata){pSetLastError(incoming_error);return(HRESULT)0x80004005L;}LogRecordV6 r;common_fill(&r,REC_DI_GET_DATA_CALL,ret);UINT32 call_id=(UINT32)__sync_add_and_fetch(&g_call_id,1);r.p.data_call.call_id=call_id;r.p.data_call.device_ptr=(UINT32)(ULONG_PTR)self;r.p.data_call.cb_object_data=cbobj;r.p.data_call.data_ptr=(UINT32)(ULONG_PTR)data;r.p.data_call.count_ptr=(UINT32)(ULONG_PTR)count;r.p.data_call.flags=flags;r.p.data_call.format_kind=w->format_kind;r.p.data_call.x_offset=w->x_offset;r.p.data_call.y_offset=w->y_offset;r.p.data_call.z_offset=w->z_offset;if(count&&readable_span(count,sizeof(DWORD)))r.p.data_call.count_before=*count;pSetLastError(incoming_error);HRESULT hr=w->original_getdata(self,cbobj,data,count,flags);DWORD api_error=pGetLastError();r.p.data_call.result_hr=(UINT32)hr;r.p.data_call.last_error=api_error;DWORD n=0;if(count&&readable_span(count,sizeof(DWORD))){n=*count;r.p.data_call.count_after=n;}DWORD parse_n=n;if(parse_n>512){parse_n=512;r.p.data_call.truncated=1;}if(hr_success(hr)&&data&&cbobj>=16&&parse_n&&readable_span(data,(SIZE_T)cbobj*parse_n)){r.p.data_call.buffer_readable=1;for(DWORD i=0;i<parse_n;++i){const BYTE* p=(const BYTE*)data+(SIZE_T)i*cbobj;UINT32 ofs=*(const UINT32*)(p+0),raw=*(const UINT32*)(p+4),ts=*(const UINT32*)(p+8),seq=*(const UINT32*)(p+12);UINT32 axis=0;if(ofs==w->x_offset)axis=1;else if(ofs==w->y_offset)axis=2;else if(ofs==w->z_offset)axis=3;if(axis){LogRecordV6 e;common_fill(&e,REC_DI_GET_DATA_AXIS,ret);e.p.data_axis.call_id=call_id;e.p.data_axis.device_ptr=(UINT32)(ULONG_PTR)self;e.p.data_axis.event_index=i;e.p.data_axis.object_offset=ofs;e.p.data_axis.raw_data=raw;e.p.data_axis.time_stamp=ts;e.p.data_axis.sequence=seq;e.p.data_axis.axis_kind=axis;e.p.data_axis.result_hr=(UINT32)hr;e.p.data_axis.delta=(INT32)raw;e.c.qpc_end=qpc_now();enqueue_record(&e);++r.p.data_call.axis_events;__sync_add_and_fetch(&g_getdata_axis_events,1);if((axis==1||axis==2)&&raw!=0){__sync_add_and_fetch(&g_getdata_nonzero,1);}}++r.p.data_call.parsed_objects;}}
...
static void writer_process_record(const LogRecordV6* r){
    ...
    else if(r->c.type==REC_DI8_CREATE){status_text("DirectInput8Create caller=");status_module_plus_rva(r->c.caller_module_base,r->c.caller_rva);status_text(" hr=");status_hex32(r->p.di8.result_hr);status_text(" object=");status_hex32(r->p.di8.object_ptr);status_text(" wrapped=");status_dec(r->p.di8.wrapped);status_text("\r\n");}
    else if(r->c.type==REC_DI_CREATE_DEVICE){status_text("CreateDevice caller=");status_module_plus_rva(r->c.caller_module_base,r->c.caller_rva);status_text(" mouse_guid=");status_dec(r->p.create_device.is_mouse_guid);status_text(" hr=");status_hex32(r->p.create_device.result_hr);status_text(" device=");status_hex32(r->p.create_device.device_ptr);status_text(" wrapped=");status_dec(r->p.create_device.wrapped);status_text("\r\n");}
...
static DWORD WINAPI init_thread(LPVOID){
    ...
    g_status=CreateFileA(status_path,GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);status_text("Dead Space 3 DirectInput + D820 Probe v0.6\r\n");status_text("MODE: diagnostic only; no input replacement, no Raw Input registration, no WndProc subclassing.\r\n");status_text("LOGGER: DirectInput/D820 producer paths are memory-only; disk I/O is on a separate writer thread.\r\n");
    ...
    LogHeaderV6 h;zero_bytes(&h,sizeof(h));h.magic[0]='D';h.magic[1]='S';h.magic[2]='3';h.magic[3]='D';h.magic[4]='I';h.magic[5]='6';h.magic[6]='0';h.version=60;h.header_size=sizeof(h);h.record_size=sizeof(LogRecordV6);h.flags=7;h.qpc_frequency=fq.QuadPart;h.image_timestamp=ts;h.image_size=image_size;h.text_rva=text_rva;h.queue_capacity_records=kQueueCapacity;h.flush_interval_ms=kFlushIntervalMs;h.module_rescan_ms=kModuleRescanMs;h.rawreg_poll_ms=kRawRegPollMs;DWORD wrote=0;if(!WriteFile(g_log,&h,sizeof(h),&wrote,0)||wrote!=sizeof(h)){status_text("REFUSED: failed to write binary header.\r\n");return 0;}
    ...
    status_text("EXPECTED: DIRECTINPUT_MOUSE_CONFIRMED appears only after a non-zero X/Y mouse delta is returned by GetDeviceState or GetDeviceData.\r\n");return 0;
}
