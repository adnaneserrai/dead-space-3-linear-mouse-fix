...
def main():
    ...
    if magic!=b'DS3DI60\0' or version!=60 or header_size!=HEADER.size or record_size!=RECORD_SIZE:
    ...
    print('DS3 DirectInput + D820 Probe v0.6')
    ...
    di8_rows=[]; create_rows=[]; fmt_rows=[]; coop_rows=[]; acq_rows=[]; state_rows=[]; data_call_rows=[]; axis_rows=[]; d820_rows=[]; rawreg_rows=[]; iat_rows=[]
    ...
    for r in records:
        ...
        if typ==2:
            ...
            row={'time_s':f'{t:.9f}','caller':caller,'thread':r['thread'],'version':f'0x{u[2]:08X}','iid':guid,'hr':f'0x{u[6]:08X}','last_error':u[7],'object':f'0x{u[8]:08X}','wrapped':u[9]}; di8_rows.append(row)
        elif typ==3:
            u=struct.unpack_from('<10I',raw,p); guid=guid_text(raw[p+40:p+56])
            create_rows.append({'time_s':f'{t:.9f}','caller':caller,'thread':r['thread'],'guid':guid,'hr':f'0x{u[5]:08X}','last_error':u[6],'device':f'0x{u[7]:08X}','is_mouse_guid':u[8],'wrapped':u[9]})
        elif typ==4:
            ...
            row={'time_s':f'{t:.9f}','caller':caller,'thread':r['thread'],'device':f'0x{u[0]:08X}','cb_data':u[1],'hr':f'0x{u[3]:08X}','last_error':u[4],'format_kind':u[5],'x_offset':f'0x{u[6]:08X}','y_offset':f'0x{u[7]:08X}','z_offset':f'0x{u[8]:08X}','buffer_readable':u[9],'valid_xyz':u[10],'x':u[11],'y':u[12],'z':u[13]}; state_rows.append(row)
            ...
            row={'time_s':f'{t:.9f}','caller':caller,'thread':r['thread'],'self':f'0x{selfp:08X}','dt':dt,'in_x':ix,'in_y':iy,'out_x':ox,'out_y':oy,'result':result}; d820_rows.append(row)
            ...
            latest_stats=struct.unpack_from('<26I',raw,p)
        elif typ==15:
    ...
    if not di8_rows: print('  calls=0')
        ...
        print(f'  calls={len(di8_rows)} wrapped={sum(int(x["wrapped"]) for x in di8_rows)}')
        for k,v in Counter(x['caller'] for x in di8_rows).items(): print(f'  caller={k} calls={v}')
    print('\nCreateDevice:')
    if not create_rows: print('  calls=0')
    else:
        print(f'  calls={len(create_rows)} mouse_guid={sum(int(x["is_mouse_guid"]) for x in create_rows)} mouse_wrapped={sum(int(x["wrapped"]) for x in create_rows)}')
        for x in create_rows: print(f"  caller={x['caller']} guid={x['guid']} mouse={x['is_mouse_guid']} wrapped={x['wrapped']} hr={x['hr']}")
    print('\nMouse data format:')
    ...
    if latest_stats:
        names=['enqueued','dropped','dropped_di','dropped_d820','highwater','write_errors','di8create','create_device','mouse_devices','setformat','setcoop','acquire','unacquire','getstate','getstate_packets','getstate_nonzero','getdata','getdata_axis','getdata_nonzero','d820','modules','rawreg_polls','rawreg_changes','rawreg_entries','iat_patched','iat_conflict']
    ...
    write_csv(str(prefix)+'_modules.csv',module_rows); write_csv(str(prefix)+'_di8_create.csv',di8_rows); write_csv(str(prefix)+'_create_device.csv',create_rows)
    ...
    write_csv(str(prefix)+'_get_data_axis.csv',axis_rows); write_csv(str(prefix)+'_d820.csv',d820_rows); write_csv(str(prefix)+'_raw_registration.csv',rawreg_rows); write_csv(str(prefix)+'_iat.csv',iat_rows)
