// Dead Space 3 DirectInput + D820 + AB9830 Experimental Bypass v0.8
// x86 / no CRT / experimental A/B validation only.
//
...
// Optional INI-controlled experiment: only on the verified camera path, only after
// a fresh non-zero DirectInput mouse delta on the same thread, and only when the
// pre-AB9830 magnitude exceeds 1, restore pre_x/pre_y after calling vanilla AB9830.
// Default is OFF. No Raw Input device is registered. No WndProc is subclassed.
// deadspace3.exe is never modified on disk.
...
// -----------------------------------------------------------------------------
// Binary log format v8.0. Every record is 512 bytes.
// -----------------------------------------------------------------------------
enum RecordTypeV6 {
    REC_MODULE=1,
    REC_DI8_CREATE=2,
...
struct MetaPayloadV6 {
    ...
    UINT32 bypass_enabled,fresh_window_us,reserved1,reserved2;
};
...
extern "C" void __attribute__((fastcall)) hook_ab9830(void* self,void*,float* x,float* y,float curve0,float curve1){
    ...
    if(targeted){
        ...
        if(g_bypass_enabled&&fresh_nonzero&&pre_over&&x&&y){*x=pre_x;*y=pre_y;r.p.ab9830.flags|=16U;__sync_add_and_fetch(&g_bypass_applied,1);}
...
static DWORD WINAPI init_thread(LPVOID){
    ...
    g_status=CreateFileA(status_path,GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);status_text("Dead Space 3 DirectInput + D820 + AB9830 Experimental Bypass v0.8\r\n");status_text("MODE: experimental A/B validation; bypass default OFF; no Raw Input registration, no WndProc subclassing.\r\n");status_text("LOGGER: DirectInput/D820 producer paths are memory-only; disk I/O is on a separate writer thread.\r\n");
    if(!resolve_apis()){status_text("REFUSED: required Win32 API resolution failed. Nothing hooked.\r\n");return 0;}
    char ini_path[1024];build_path(ini_path,1024,"DS3_DirectInputProbe.ini");if(pGetPrivateProfileIntA)g_bypass_enabled=pGetPrivateProfileIntA("Mouse","EnableMouseAB9830Bypass",0,ini_path)?1:0;else g_bypass_enabled=0;status_text("CONFIG EnableMouseAB9830Bypass=");status_dec((UINT32)g_bypass_enabled);status_text(" freshMouseWindowUs=");status_dec(kFreshMouseWindowUs);status_text("\r\n");
    ...
    LogHeaderV6 h;zero_bytes(&h,sizeof(h));h.magic[0]='D';h.magic[1]='S';h.magic[2]='3';h.magic[3]='D';h.magic[4]='I';h.magic[5]='8';h.magic[6]='0';h.version=80;h.header_size=sizeof(h);h.record_size=sizeof(LogRecordV6);h.flags=31;h.qpc_frequency=fq.QuadPart;h.image_timestamp=ts;h.image_size=image_size;h.text_rva=text_rva;h.queue_capacity_records=kQueueCapacity;h.flush_interval_ms=kFlushIntervalMs;h.module_rescan_ms=kModuleRescanMs;h.rawreg_poll_ms=kRawRegPollMs;DWORD wrote=0;if(!WriteFile(g_log,&h,sizeof(h),&wrote,0)||wrote!=sizeof(h)){status_text("REFUSED: failed to write binary header.\r\n");return 0;}
    ...
    status_text("EXPECTED: DIRECTINPUT_MOUSE_CONFIRMED + AB9830_CAMERA_PATH_CONFIRMED; bypass is applied only when enabled + targeted + fresh non-zero DI + pre magnitude >1.\r\n");return 0;
}
