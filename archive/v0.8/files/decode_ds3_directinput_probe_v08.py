...
def main():
    ...
    if magic!=b'DS3DI80\0' or version!=80 or header_size!=HEADER.size or record_size!=RECORD_SIZE:
    ...
    print('DS3 DirectInput + D820 + AB9830 Experimental Bypass v0.8')
    ...
    for r in records:
        ...
        if typ==2:
            ...
            u=struct.unpack_from('<8I8f2i2I',raw,p)
            flags_ab=u[7]; pre_mag=math.hypot(u[10],u[11]); vanilla_mag=math.hypot(u[12],u[13]); effective_mag=math.hypot(u[14],u[15])
            age_us=(u[19]*1_000_000.0/qpc) if qpc else float('nan')
            ab9830_rows.append({'time_s':f'{t:.9f}','qpc_begin':r['qpc_begin'],'qpc_end':r['qpc_end'],'caller':caller,'caller_rva':r['caller_rva'],'thread':r['thread'],
                ...
                'target_d820_return_rva':f'0x{u[5]:08X}','path_gate_depth':u[6],'flags':f'0x{flags_ab:08X}','curve_arg0':u[8],'curve_arg1':u[9],
                'pre_x':u[10],'pre_y':u[11],'pre_magnitude':pre_mag,'vanilla_post_x':u[12],'vanilla_post_y':u[13],'vanilla_post_magnitude':vanilla_mag,
                'effective_post_x':u[14],'effective_post_y':u[15],'effective_post_magnitude':effective_mag,'fresh_di_x':u[16],'fresh_di_y':u[17],
                'fresh_generation':u[18],'fresh_age_ticks':u[19],'fresh_age_us':age_us,'fresh_read':int(bool(flags_ab&1)),'fresh_nonzero':int(bool(flags_ab&2)),
                'pre_over_1':int(bool(flags_ab&4)),'bypass_enabled':int(bool(flags_ab&8)),'bypass_applied':int(bool(flags_ab&16)),
                'vanilla_post_near_1':int(0.999<=vanilla_mag<=1.001)})
        elif typ==12:
            ...
            latest_stats=struct.unpack_from('<37I',raw,p)
        elif typ==15:
    ...
    if ab9830_rows:
        pre_over=sum(int(x['pre_over_1']) for x in ab9830_rows); post_unit=sum(int(x['vanilla_post_near_1']) for x in ab9830_rows); bypassed=sum(int(x['bypass_applied']) for x in ab9830_rows); fresh=sum(int(x['fresh_nonzero']) for x in ab9830_rows)
        print(f'  pre_magnitude>1={pre_over} vanilla_post_magnitude~1={post_unit} fresh_nonzero={fresh} bypass_applied={bypassed}')
        print(f"  max_pre_magnitude={max(float(x['pre_magnitude']) for x in ab9830_rows):.9g} max_vanilla_post_magnitude={max(float(x['vanilla_post_magnitude']) for x in ab9830_rows):.9g} max_effective_post_magnitude={max(float(x['effective_post_magnitude']) for x in ab9830_rows):.9g}")
    ...
    if ab9830_rows:
        ...
        for a in ab9830_rows:
            ...
            row={'time_s':a['time_s'],'thread':tid,'di_x':sr['x'] if sr else '', 'di_y':sr['y'] if sr else '',
                 ...
                 'pre_x':a['pre_x'],'pre_y':a['pre_y'],'pre_magnitude':a['pre_magnitude'],'vanilla_post_x':a['vanilla_post_x'],'vanilla_post_y':a['vanilla_post_y'],'vanilla_post_magnitude':a['vanilla_post_magnitude'],'effective_post_x':a['effective_post_x'],'effective_post_y':a['effective_post_y'],'effective_post_magnitude':a['effective_post_magnitude'],'fresh_di_x':a['fresh_di_x'],'fresh_di_y':a['fresh_di_y'],'fresh_age_us':a['fresh_age_us'],'bypass_applied':a['bypass_applied'],
            ...
            if dr:
                gaps_d.append(float(row['ab_to_d820_ms'])); diffs.append(max(abs(float(a['effective_post_x'])-float(dr['in_x'])),abs(float(a['effective_post_y'])-float(dr['in_y'])))); matched+=1
            pipeline_rows.append(row)
        print('\nPipeline synchronization:')
        print(f'  AB9830->D820 matched={matched}/{len(ab9830_rows)}')
        if diffs: print(f'  max_abs(effective_AB9830 - D820_input)={max(diffs):.9g}')
