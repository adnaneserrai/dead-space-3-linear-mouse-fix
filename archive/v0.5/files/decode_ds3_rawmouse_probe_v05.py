#!/usr/bin/env python3
"""Decode/analyse DS3_RawMouseProbe v0.5 binary capture.
...
def parse(path: Path):
    ...
    if len(raw) < HEADER.size:
        raise SystemExit("Capture too small for v0.5 header")
    ...
    if magic != b"DS3RMP5\0" or version != 5 or header_size != HEADER.size or record_size != RECORD_SIZE:
...
def main() -> int:
    ...
    print("DS3 Raw Mouse Probe v0.5")
    ...
    for r in records:
        ...
        b=r["_buf"]; base=u32(b,40); rn=u32(b,44); gn=u32(b,48); cn=u32(b,52)
        iat_rows.append((base,rn,gn,cn))
    if iat_rows:
        print("\nIAT observer:")
        for base,rn,gn,cn in iat_rows:
            if rn or gn or cn:
                print(f"  {modules.get(base,{}).get('name',f'0x{base:08X}')}: Register={rn} GetRawInputData={gn} conflicts={cn}")
    ...
    for r in records:
        ...
        keys=["enqueued","dropped","dropped_api","dropped_d820","highwater","write_errors","reg_calls","getraw_calls","getraw_query","getraw_actual","raw_packets","raw_nonzero","raw_errors","d820","modules","iat_reg","iat_raw","iat_conflicts"]
