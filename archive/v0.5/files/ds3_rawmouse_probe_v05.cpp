// Dead Space 3 Raw Mouse Probe v0.5
...
//     and USER32!GetRawInputData in every loaded module, with periodic rescans.
...
struct RAWINPUT_X { RAWINPUTHEADER_X header; union { RAWMOUSE_X mouse; BYTE raw[32]; } data; };
...
static_assert(sizeof(RAWINPUT_X)>=40,"x86 RAWINPUT layout mismatch");
...
// -----------------------------------------------------------------------------
// Binary format v5. Every record is exactly 384 bytes.
// -----------------------------------------------------------------------------
enum RecordType {
    RECORD_MODULE = 1,
    RECORD_REG_CALL = 2,
...
static BOOL resolve_apis(){
    ...
    return pRealRegisterRawInputDevices&&pRealGetRawInputData;
}
...
extern "C" BOOL WINAPI hook_RegisterRawInputDevices(const RAWINPUTDEVICE_X* devices,UINT num,UINT cb){
    void* ret=__builtin_return_address(0);LogRecordV5 base;common_fill(&base,RECORD_REG_CALL,ret);UINT32 call_id=(UINT32)__sync_add_and_fetch(&g_api_call_id,1);
    ...
    DWORD entry_error=pGetLastError();(void)entry_error;
    BOOL result=pCallRegisterRawInputDevices(devices,num,cb);DWORD after_error=pGetLastError();
...
extern "C" UINT WINAPI hook_GetRawInputData(HRAWINPUT hraw,UINT cmd,LPVOID data,UINT* pcb,UINT cb_header){
    void* ret=__builtin_return_address(0);UINT size_before=0;BOOL pcb_ok=pcb&&readable_span(pcb,sizeof(UINT));if(pcb_ok)size_before=*pcb;
    ...
    long long begin=qpc_now();UINT result=pCallGetRawInputData(hraw,cmd,data,pcb,cb_header);DWORD after_error=pGetLastError();long long end=qpc_now();UINT size_after=pcb_ok?*pcb:0;
    if(cmd==RID_INPUT&&data){
        ...
        BOOL valid_buffer=FALSE;if(result!=UINT_ERROR&&result>=sizeof(RAWINPUTHEADER_X)&&readable_span(data,result)){valid_buffer=TRUE;r.p.raw.buffer_readable=1;RAWINPUT_X* ri=(RAWINPUT_X*)data;r.p.raw.dw_type=ri->header.dwType;r.p.raw.dw_size=ri->header.dwSize;r.p.raw.hdevice=(UINT32)(ULONG_PTR)ri->header.hDevice;r.p.raw.raw_wparam=(UINT32)ri->header.wParam;
            if(ri->header.dwType==RIM_TYPEMOUSE&&result>=sizeof(RAWINPUT_X)&&ri->header.dwSize>=sizeof(RAWINPUT_X)){r.p.raw.valid_mouse=1;r.p.raw.us_flags=ri->data.mouse.usFlags;r.p.raw.button_flags=ri->data.mouse.usButtonFlags;r.p.raw.button_data=ri->data.mouse.usButtonData;r.p.raw.dx=ri->data.mouse.lLastX;r.p.raw.dy=ri->data.mouse.lLastY;__sync_add_and_fetch(&g_raw_mouse_packets,1);if(!(ri->data.mouse.usFlags&MOUSE_MOVE_ABSOLUTE)&&(ri->data.mouse.lLastX||ri->data.mouse.lLastY))__sync_add_and_fetch(&g_raw_mouse_nonzero,1);}}
...
static void patch_iat_module(BYTE* base,UINT32* reg_new,UINT32* raw_new,UINT32* conflicts){
    *reg_new=*raw_new=*conflicts=0;if(!base||base[0]!='M'||base[1]!='Z')return;DWORD e=*(DWORD*)(base+0x3C);BYTE* pe=base+e;if(*(DWORD*)pe!=0x00004550UL||*(WORD*)(pe+4)!=0x014C)return;BYTE* opt=pe+24;if(*(WORD*)opt!=0x010B)return;
    ...
    for(;d+20<=dend;d+=20){DWORD oft=*(DWORD*)(d+0);DWORD name=*(DWORD*)(d+12);DWORD ft=*(DWORD*)(d+16);if(!oft&&!name&&!ft)break;if(!ft)continue;UINT32* iat=(UINT32*)(base+ft);UINT32* names=oft?(UINT32*)(base+oft):0;
        for(UINT32 i=0;i<4096;++i){UINT32 cur=iat[i];if(!cur)break;const char* fn=0;if(names&&names[i]&&!is_ordinal(names[i]))fn=(const char*)(base+names[i]+2);
            BOOL want_reg=fn&&streq(fn,"RegisterRawInputDevices");BOOL want_raw=fn&&streq(fn,"GetRawInputData");
            if(!fn){if(cur==(UINT32)(ULONG_PTR)pRealRegisterRawInputDevices)want_reg=TRUE;if(cur==(UINT32)(ULONG_PTR)pRealGetRawInputData)want_raw=TRUE;}
...
static void emit_iat_record(UINT32 base,UINT32 rn,UINT32 gn,UINT32 cn){if(!rn&&!gn&&!cn)return;LogRecordV5 r;common_fill(&r,RECORD_IAT,0);r.c.caller_address=0;r.c.caller_module_base=base;r.c.caller_rva=0;r.p.iat.module_base=base;r.p.iat.register_slots_new=rn;r.p.iat.raw_slots_new=gn;r.p.iat.conflicts_new=cn;r.c.qpc_end=qpc_now();enqueue_record(&r);}
...
static void scan_modules_and_patch(){
    ...
    if(pModule32FirstA(snap,&me)){do{UINT32 base=(UINT32)(ULONG_PTR)me.modBaseAddr;cache_module(base,me.modBaseSize,me.szModule,me.szExePath);if((HMODULE)(ULONG_PTR)base!=g_module){UINT32 rn=0,gn=0,cn=0;patch_iat_module(me.modBaseAddr,&rn,&gn,&cn);emit_iat_record(base,rn,gn,cn);}}while(pModule32NextA(snap,&me));}
    CloseHandle(snap);
}
...
static void writer_process_record(const LogRecordV5* r){
    if(r->c.type==RECORD_REG_DEVICE){status_text("REG caller=");status_module_plus_rva(r->c.caller_module_base,r->c.caller_rva);status_text(" tid=");status_dec(r->c.thread_id);status_text(" usagePage=");status_dec(r->p.reg_device.usage_page);status_text(" usage=");status_dec(r->p.reg_device.usage);status_text(" target=");status_hex32(r->p.reg_device.hwnd_target);status_text(" flags=");status_hex32(r->p.reg_device.flags);status_text(" result=");status_dec(r->p.reg_device.result);if(!r->p.reg_device.result){status_text(" error=");status_dec(r->p.reg_device.last_error);}status_text("\r\n");}
        ...
        if(!(r->p.raw.us_flags&MOUSE_MOVE_ABSOLUTE)&&(r->p.raw.dx||r->p.raw.dy)&&__sync_val_compare_and_swap(&g_status_game_raw_confirmed,0,1)==0)status_text("GAME_RAW_INPUT_CONFIRMED: valid non-zero RID_INPUT mouse data was returned to the game/process caller.\r\n");
    }
...
static DWORD WINAPI init_thread(LPVOID){
    ...
    LogHeaderV5 h;zero_bytes(&h,sizeof(h));h.magic[0]='D';h.magic[1]='S';h.magic[2]='3';h.magic[3]='R';h.magic[4]='M';h.magic[5]='P';h.magic[6]='5';h.version=5;h.header_size=sizeof(h);h.record_size=sizeof(LogRecordV5);h.flags=3;h.qpc_frequency=fq.QuadPart;h.image_timestamp=ts;h.image_size=image_size;h.text_rva=text_rva;h.queue_capacity_records=kQueueCapacity;h.flush_interval_ms=kFlushIntervalMs;h.module_rescan_ms=kModuleRescanMs;DWORD wrote=0;if(!WriteFile(g_log,&h,sizeof(h),&wrote,0)||wrote!=sizeof(h)){status_text("REFUSED: failed to write binary header.\r\n");return 0;}
    ...
    void* call_reg=0;void* call_raw=0;
    ...
    HANDLE mt=CreateThread(0,0,module_scanner_thread,0,0,0);if(mt)CloseHandle(mt);status_text("API_OBSERVER_ACTIVE: process-wide hotpatch where safely available, otherwise periodic loaded-module IAT fallback; no Raw Input receiver was registered by this probe.\r\n");
    ...
    status_text("EXPECTED: REG lines identify registration ownership; GAME_RAW_INPUT_CONFIRMED appears only after a real non-zero RID_INPUT mouse buffer is returned through GetRawInputData.\r\n");
    return 0;
}
