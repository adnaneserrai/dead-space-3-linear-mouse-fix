// Dead Space 3 Linear Mouse Fix - Experimental A/B v0.8.2
// x86 / no CRT / selective mouse-look validation build.
//
// Extends the proven v0.8.1 selective AB9830 bypass + post-vanilla D820
// counterfactual unclamp to two verified mouse-look profiles: HIP and ADS/RMB.
// D820 history/state updates remain untouched. Only on a verified profile and
// only while a same-profile per-thread mouse-tail latch is active, the hook
// reconstructs the already validated D820 post-state model and replaces only a
// clamped output with the counterfactual unclamped output preclamp*scale.
// Default is OFF. No global AB9830/D820 behavior change, no Raw Input receiver,
// no WndProc subclass, and deadspace3.exe is never modified on disk.
...
// -----------------------------------------------------------------------------
// Binary log format v8.1. Every record is 512 bytes.
// -----------------------------------------------------------------------------
enum RecordTypeV6 {
    REC_MODULE=1,
    REC_DI8_CREATE=2,
...
#pragma pack(push,1)
struct LogHeaderV6 {
    BYTE magic[8];              // "DS3LM82\0"
    UINT32 version;             // 82
    UINT32 header_size;
    UINT32 record_size;
    UINT32 flags;               // bit0 buffered writer, bit1 DI observer, bit2 passive raw-reg monitor, bit3 AB9830 observer, bit4 optional bypass supported
...
struct ProcessorPayloadV6 {
    ...
    StateSnapshotV6 pre,post; INT32 result;
    float vanilla_out_x,vanilla_out_y;
    float model_preclamp_x,model_preclamp_y,model_scale;
    UINT32 correction_flags;       // bit0 model valid, bit1 target camera caller, bit2 latch active, bit3 over limit, bit4 unclamp applied, bit5 tail correction
    UINT32 tail_generation;
    UINT32 tail_age_ticks;
};
struct AB9830PayloadV8 {
    ...
    UINT32 flags;                    // bit0 fresh DI read, bit1 fresh non-zero, bit2 pre magnitude >1, bit3 bypass enabled, bit4 bypass applied
    float curve_arg0,curve_arg1;
    float pre_x,pre_y;
    float vanilla_post_x,vanilla_post_y;
    float effective_post_x,effective_post_y;
    INT32 fresh_di_x,fresh_di_y;
    UINT32 fresh_generation;
    UINT32 fresh_age_ticks;
    UINT32 reserved[8];
};
...
struct StatsPayloadV6 {
    ...
    UINT32 bypass_enabled,bypass_applied,fresh_mouse_reads,fresh_mouse_claims,fresh_mouse_nonzero_claims,fresh_mouse_stale_rejects;
    UINT32 d820_unclamp_applied,d820_tail_applied,d820_model_failures,tail_starts,tail_timeouts,tail_stable_exits;
    UINT32 hip_ab_bypass,hip_d820_unclamp,ads_ab_bypass,ads_d820_unclamp,ads_tail;
};
struct MetaPayloadV6 {
    ...
    UINT32 bypass_enabled,fresh_window_us,tail_timeout_us,tail_stable_samples;
    UINT32 ads_abc510_return_rva,ads_d820_return_rva;
};
...
union PayloadV6 {
    ...
    MetaPayloadV6 meta; IatPayloadV6 iat; VtablePayloadV61 vtable; AB9830PayloadV8 ab9830; BYTE pad[472];
};
...
static_assert(sizeof(AB9830PayloadV8)<=472,"AB9830 payload too large");
...
static volatile LONG g_bypass_enabled=0,g_bypass_applied=0,g_fresh_mouse_reads=0,g_fresh_mouse_claims=0,g_fresh_mouse_nonzero_claims=0,g_fresh_mouse_stale_rejects=0;
static volatile LONG g_d820_unclamp_applied=0,g_d820_tail_applied=0,g_d820_model_failures=0,g_tail_starts=0,g_tail_timeouts=0,g_tail_stable_exits=0;
static volatile LONG g_hip_ab_bypass=0,g_hip_d820_unclamp=0,g_ads_ab_bypass=0,g_ads_d820_unclamp=0,g_ads_tail=0;
...
enum MouseLookProfileV82 { PROFILE_NONE=0, PROFILE_HIP=1, PROFILE_ADS=2 };
...
struct PathGateSlotV7 { volatile LONG thread_id; volatile LONG hip_depth; volatile LONG ads_depth; };
static PathGateSlotV7 g_path_gate[32];
struct FreshMouseSlotV8 {
...
static FreshMouseSlotV8 g_fresh_mouse[32];

struct MouseTailSlotV81 {
    volatile LONG thread_id;
...
static MouseTailSlotV81 g_mouse_tail[32];

struct D820ModelV81 {
    BOOL valid;
    float preclamp_x,preclamp_y,scale,limit;
};
...
static MouseTailSlotV81* mouse_tail_slot(UINT32 tid,BOOL create){
...
static void mouse_tail_start(UINT32 tid,long long now,LONG profile){
    MouseTailSlotV81* s=mouse_tail_slot(tid,TRUE);if(!s)return;
    s->last_bypass_qpc=now;s->stable_under_count=0;s->profile=profile;s->active=1;__sync_add_and_fetch(&s->generation,1);__sync_add_and_fetch(&g_tail_starts,1);
}
static BOOL reconstruct_d820_model_v81(const StateSnapshotV6* post,D820ModelV81* out){
...
static LONG path_gate_depth(UINT32 tid,LONG profile){PathGateSlotV7* s=path_gate_slot(tid,FALSE);if(!s)return 0;return profile==PROFILE_HIP?s->hip_depth:(profile==PROFILE_ADS?s->ads_depth:0);}
static LONG path_gate_profile(UINT32 tid){PathGateSlotV7* s=path_gate_slot(tid,FALSE);if(!s)return PROFILE_NONE;if(s->ads_depth>0)return PROFILE_ADS;if(s->hip_depth>0)return PROFILE_HIP;return PROFILE_NONE;}
static FreshMouseSlotV8* fresh_mouse_slot(UINT32 tid,BOOL create){
...
static void fresh_mouse_publish(UINT32 tid,long long qpc,INT32 x,INT32 y){
    FreshMouseSlotV8* s=fresh_mouse_slot(tid,TRUE);if(!s)return;
    s->qpc_end=qpc;s->x=x;s->y=y;__sync_synchronize();s->valid=1;__sync_add_and_fetch(&s->generation,1);__sync_add_and_fetch(&g_fresh_mouse_reads,1);
}
struct FreshMouseClaimV8 { BOOL fresh; BOOL nonzero; INT32 x,y; UINT32 generation,age_ticks; };
static FreshMouseClaimV8 fresh_mouse_claim(UINT32 tid,long long now){
    FreshMouseClaimV8 c;c.fresh=FALSE;c.nonzero=FALSE;c.x=0;c.y=0;c.generation=0;c.age_ticks=0;FreshMouseSlotV8* s=fresh_mouse_slot(tid,FALSE);if(!s||!s->valid)return c;
    ...
    s->consumed_generation=gen;c.fresh=TRUE;c.x=s->x;c.y=s->y;c.generation=(UINT32)gen;c.age_ticks=(ticks>0xFFFFFFFFLL)?0xFFFFFFFFU:(UINT32)ticks;__sync_add_and_fetch(&g_fresh_mouse_claims,1);
    BOOL within=(g_qpc_frequency>0&&ticks>=0&&(ticks*1000000LL)<=g_qpc_frequency*(long long)kFreshMouseWindowUs);if(!within){__sync_add_and_fetch(&g_fresh_mouse_stale_rejects,1);return c;}
    c.nonzero=(c.x!=0||c.y!=0);if(c.nonzero)__sync_add_and_fetch(&g_fresh_mouse_nonzero_claims,1);return c;
}
...
static BOOL resolve_apis(){
    ...
    report_missing_api("KERNEL32!Module32First",(const void*)pModule32FirstA);report_missing_api("KERNEL32!Module32Next",(const void*)pModule32NextA);if(!pGetPrivateProfileIntA)status_text("OPTIONAL_API_MISSING: KERNEL32!GetPrivateProfileIntA; bypass forced OFF.\r\n");
...
// -----------------------------------------------------------------------------
// HIP + ADS camera-path gate and selective AB9830 experiment.
// No global AB9830 behavior change: both profiles call vanilla first and restore
// pre-clamp X/Y only with an unconsumed fresh DirectInput mouse delta on the same
// thread and pre magnitude > 1.
// -----------------------------------------------------------------------------
extern "C" UINT32 __attribute__((fastcall)) hook_abc510(void* self,void*,UINT32 a1,UINT32 a2,UINT32 a3,UINT32 a4,UINT32 a5,UINT32 a6,UINT32 a7,UINT32 a8,UINT32 a9){
    ...
    LONG profile=PROFILE_NONE;
    if(g_base&&ret==(UINT32)(ULONG_PTR)(g_base+kHipABC510ReturnRva))profile=PROFILE_HIP;
    else if(g_base&&ret==(UINT32)(ULONG_PTR)(g_base+kAdsABC510ReturnRva))profile=PROFILE_ADS;
    PathGateSlotV7* slot=0;
    if(profile!=PROFILE_NONE){slot=path_gate_slot(tid,TRUE);if(slot){if(profile==PROFILE_HIP)__sync_add_and_fetch(&slot->hip_depth,1);else __sync_add_and_fetch(&slot->ads_depth,1);__sync_add_and_fetch(&g_abc510_target_calls,1);}}
    UINT32 result=g_abc510(self,a1,a2,a3,a4,a5,a6,a7,a8,a9);
    if(slot){if(profile==PROFILE_HIP)__sync_sub_and_fetch(&slot->hip_depth,1);else if(profile==PROFILE_ADS)__sync_sub_and_fetch(&slot->ads_depth,1);}
    return result;
}
...
extern "C" void __attribute__((fastcall)) hook_ab9830(void* self,void*,float* x,float* y,float curve0,float curve1){
    ...
    LONG profile=path_gate_profile(tid);LONG depth=path_gate_depth(tid,profile);
    BOOL targeted=(g_base&&profile!=PROFILE_NONE&&depth>0&&ret==(UINT32)(ULONG_PTR)(g_base+kTargetAB9830ReturnRva));
    LogRecordV6 r;float pre_x=0.0f,pre_y=0.0f;FreshMouseClaimV8 claim;claim.fresh=FALSE;claim.nonzero=FALSE;claim.x=claim.y=0;claim.generation=claim.age_ticks=0;BOOL pre_over=FALSE;
    if(targeted){
        ...
        r.p.ab9830.direct_return_rva=kTargetAB9830ReturnRva;
        r.p.ab9830.parent_return_rva=(profile==PROFILE_HIP)?kHipABC510ReturnRva:kAdsABC510ReturnRva;
        r.p.ab9830.target_d820_return_rva=(profile==PROFILE_HIP)?kHipD820ReturnRva:kAdsD820ReturnRva;
        r.p.ab9830.path_gate_depth=(UINT32)depth;
        r.p.ab9830.curve_arg0=curve0;r.p.ab9830.curve_arg1=curve1;pre_x=x?*x:0.0f;pre_y=y?*y:0.0f;r.p.ab9830.pre_x=pre_x;r.p.ab9830.pre_y=pre_y;
        float mag2=pre_x*pre_x+pre_y*pre_y;pre_over=(mag2>1.0f);claim=fresh_mouse_claim(tid,r.c.qpc_begin);
        if(claim.fresh)r.p.ab9830.flags|=1U;if(claim.fresh&&claim.nonzero&&g_qpc_frequency>0&&((long long)claim.age_ticks*1000000LL)<=g_qpc_frequency*(long long)kFreshMouseWindowUs)r.p.ab9830.flags|=2U;if(pre_over)r.p.ab9830.flags|=4U;if(g_bypass_enabled)r.p.ab9830.flags|=8U;
        r.p.ab9830.fresh_di_x=claim.x;r.p.ab9830.fresh_di_y=claim.y;r.p.ab9830.fresh_generation=claim.generation;r.p.ab9830.fresh_age_ticks=claim.age_ticks;
    }
    g_ab9830(self,x,y,curve0,curve1);
    if(targeted){
        r.p.ab9830.vanilla_post_x=x?*x:0.0f;r.p.ab9830.vanilla_post_y=y?*y:0.0f;
        BOOL fresh_nonzero=(claim.fresh&&claim.nonzero&&g_qpc_frequency>0&&((long long)claim.age_ticks*1000000LL)<=g_qpc_frequency*(long long)kFreshMouseWindowUs);
        if(g_bypass_enabled&&fresh_nonzero&&pre_over&&x&&y){
            *x=pre_x;*y=pre_y;r.p.ab9830.flags|=16U;__sync_add_and_fetch(&g_bypass_applied,1);
            if(profile==PROFILE_HIP)__sync_add_and_fetch(&g_hip_ab_bypass,1);else __sync_add_and_fetch(&g_ads_ab_bypass,1);
            mouse_tail_start(tid,qpc_now(),profile);
        }
        r.c.qpc_end=qpc_now();r.p.ab9830.effective_post_x=x?*x:0.0f;r.p.ab9830.effective_post_y=y?*y:0.0f;__sync_add_and_fetch(&g_ab9830_targeted,1);enqueue_record(&r);
    }
}

// -----------------------------------------------------------------------------
// D820: vanilla first, then selective counterfactual unclamp on the matching
// HIP or ADS mouse-look profile only. The same validated post-state model as
// v0.8.1/D820_v08_offline_model.csv is used unchanged.
// -----------------------------------------------------------------------------
extern "C" BOOL __attribute__((fastcall)) hook_processor(void* self,void*,float dt,float* x,float* y){
    void* ret=__builtin_return_address(0);LogRecordV6 r;common_fill(&r,REC_PROCESSOR,ret);
    r.p.processor.self_ptr=(UINT32)(ULONG_PTR)self;r.p.processor.object_word_00=read_u32(self,0);r.p.processor.object_word_04=read_u32(self,4);r.p.processor.dt_bits=float_bits(dt);r.p.processor.dt=dt;r.p.processor.in_x=x?*x:0.0f;r.p.processor.in_y=y?*y:0.0f;snapshot_state(self,&r.p.processor.pre);
    BOOL result=g_processor(self,dt,x,y);long long now=qpc_now();r.c.qpc_end=now;
    float vanilla_x=x?*x:0.0f,vanilla_y=y?*y:0.0f;r.p.processor.vanilla_out_x=vanilla_x;r.p.processor.vanilla_out_y=vanilla_y;
    snapshot_state(self,&r.p.processor.post);r.p.processor.result=result;
    UINT32 tid=r.c.thread_id;UINT32 ret_addr=(UINT32)(ULONG_PTR)ret;LONG profile=PROFILE_NONE;
    if(g_base&&ret_addr==(UINT32)(ULONG_PTR)(g_base+kHipD820ReturnRva))profile=PROFILE_HIP;
    else if(g_base&&ret_addr==(UINT32)(ULONG_PTR)(g_base+kAdsD820ReturnRva))profile=PROFILE_ADS;
    BOOL target=profile!=PROFILE_NONE;if(target)r.p.processor.correction_flags|=2U;
    D820ModelV81 model;model.valid=FALSE;model.preclamp_x=model.preclamp_y=model.scale=model.limit=0.0f;BOOL model_ok=result&&reconstruct_d820_model_v81(&r.p.processor.post,&model);
    if(model_ok){r.p.processor.correction_flags|=1U;r.p.processor.model_preclamp_x=model.preclamp_x;r.p.processor.model_preclamp_y=model.preclamp_y;r.p.processor.model_scale=model.scale;}
    MouseTailSlotV81* tail=mouse_tail_slot(tid,FALSE);
    if(g_bypass_enabled&&target&&tail&&tail->active&&tail->profile==profile){
        r.p.processor.correction_flags|=4U;LONG gen=tail->generation;r.p.processor.tail_generation=(UINT32)gen;long long age=now-tail->last_bypass_qpc;if(age<0)age=0;r.p.processor.tail_age_ticks=(age>0xFFFFFFFFLL)?0xFFFFFFFFU:(UINT32)age;
        BOOL timed_out=(g_qpc_frequency>0&&age*1000000LL>g_qpc_frequency*(long long)kTailTimeoutUs);
        if(timed_out){tail->active=0;tail->profile=PROFILE_NONE;tail->stable_under_count=0;__sync_add_and_fetch(&g_tail_timeouts,1);}
        else if(model_ok){
            float lim=absf_local(model.limit);BOOL over=(absf_local(model.preclamp_x)>lim||absf_local(model.preclamp_y)>lim);if(over)r.p.processor.correction_flags|=8U;
            if(over&&x&&y){BOOL tail_only=(tail->seen_generation==gen);float corrected_x=model.preclamp_x*model.scale,corrected_y=model.preclamp_y*model.scale;*x=corrected_x;*y=corrected_y;r.p.processor.correction_flags|=16U;
                if(profile==PROFILE_HIP)__sync_add_and_fetch(&g_hip_d820_unclamp,1);else __sync_add_and_fetch(&g_ads_d820_unclamp,1);
                if(tail_only){r.p.processor.correction_flags|=32U;__sync_add_and_fetch(&g_d820_tail_applied,1);if(profile==PROFILE_ADS)__sync_add_and_fetch(&g_ads_tail,1);}tail->seen_generation=gen;tail->stable_under_count=0;__sync_add_and_fetch(&g_d820_unclamp_applied,1);
            }
            else{LONG stable=__sync_add_and_fetch(&tail->stable_under_count,1);if(stable>=(LONG)kTailStableSamples){tail->active=0;tail->profile=PROFILE_NONE;tail->stable_under_count=0;__sync_add_and_fetch(&g_tail_stable_exits,1);}}
        }else __sync_add_and_fetch(&g_d820_model_failures,1);
    }
    r.p.processor.out_x=x?*x:vanilla_x;r.p.processor.out_y=y?*y:vanilla_y;__sync_add_and_fetch(&g_d820_records,1);enqueue_record(&r);return result;
}
...
static void writer_process_record(const LogRecordV6* r){
    ...
    if(r->c.type==REC_AB9830&&(r->p.ab9830.flags&16U)){static volatile LONG once=0;if(__sync_val_compare_and_swap(&once,0,1)==0)status_text("MOUSE_AB9830_BYPASS_APPLIED_CONFIRMED\r\n");}
    if(r->c.type==REC_PROCESSOR&&(r->p.processor.correction_flags&16U)){static volatile LONG once2=0;if(__sync_val_compare_and_swap(&once2,0,1)==0)status_text("MOUSE_D820_UNCLAMP_APPLIED_CONFIRMED\r\n");}
...
static DWORD WINAPI init_thread(LPVOID){
    char status_path[1024],log_path[1024];build_path(status_path,1024,"DS3_LinearMouseFix.status.txt");build_path(log_path,1024,"DS3_LinearMouseFix.bin");
    g_status=CreateFileA(status_path,GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);status_text("Dead Space 3 Linear Mouse Fix - Experimental A/B v0.8.2\r\n");status_text("MODE: selective AB9830 bypass + selective D820 unclamp; default OFF.\r\n");status_text("LOGGER: DirectInput/D820 producer paths are memory-only; disk I/O is on a separate writer thread.\r\n");
    if(!resolve_apis()){status_text("REFUSED: required Win32 API resolution failed. Nothing hooked.\r\n");return 0;}
    char ini_path[1024];build_path(ini_path,1024,"DS3_LinearMouseFix.ini");if(pGetPrivateProfileIntA)g_bypass_enabled=pGetPrivateProfileIntA("Mouse","EnableLinearMouseFix",0,ini_path)?1:0;else g_bypass_enabled=0;status_text("CONFIG EnableLinearMouseFix=");status_dec((UINT32)g_bypass_enabled);status_text(" freshMouseWindowUs=");status_dec(kFreshMouseWindowUs);status_text(" tailTimeoutUs=");status_dec(kTailTimeoutUs);status_text(" tailStableSamples=");status_dec(kTailStableSamples);status_text("\r\n");
    ...
    g_log=CreateFileA(log_path,GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);if(g_log==INVALID_HANDLE_VALUE){status_text("REFUSED: cannot create DS3_LinearMouseFix.bin. Nothing hooked.\r\n");return 0;}
    LogHeaderV6 h;zero_bytes(&h,sizeof(h));h.magic[0]='D';h.magic[1]='S';h.magic[2]='3';h.magic[3]='L';h.magic[4]='M';h.magic[5]='8';h.magic[6]='2';h.version=82;h.header_size=sizeof(h);h.record_size=sizeof(LogRecordV6);h.flags=31;h.qpc_frequency=fq.QuadPart;h.image_timestamp=ts;h.image_size=image_size;h.text_rva=text_rva;h.queue_capacity_records=kQueueCapacity;h.flush_interval_ms=kFlushIntervalMs;h.module_rescan_ms=kModuleRescanMs;h.rawreg_poll_ms=kRawRegPollMs;DWORD wrote=0;if(!WriteFile(g_log,&h,sizeof(h),&wrote,0)||wrote!=sizeof(h)){status_text("REFUSED: failed to write binary header.\r\n");return 0;}
    ...
    if(abc_matches==1&&curve_matches==1&&abc_target&&curve_target){
        BYTE* hip_call=g_base+0x0033F3E8U;BYTE* ads_call=g_base+0x00149F12U;BYTE* curve_call=g_base+0x006BC594U;BYTE* hip_d820_call=g_base+0x0033F402U;BYTE* ads_d820_call=g_base+0x00149F29U;BOOL hip_link=(rel32_call_target(hip_call)==abc_target);BOOL ads_link=(rel32_call_target(ads_call)==abc_target);BOOL curve_link=(rel32_call_target(curve_call)==curve_target);BOOL hip_d820_link=(target&&rel32_call_target(hip_d820_call)==target);BOOL ads_d820_link=(target&&rel32_call_target(ads_d820_call)==target);BOOL links_ok=hip_link&&ads_link&&curve_link&&hip_d820_link&&ads_d820_link;
        status_text("CAMERA_PATH_LINKS verified=");status_dec(links_ok?1U:0U);status_text(" HIP abc510_return=");status_hex32(kHipABC510ReturnRva);status_text(" d820_return=");status_hex32(kHipD820ReturnRva);status_text(" ADS abc510_return=");status_hex32(kAdsABC510ReturnRva);status_text(" d820_return=");status_hex32(kAdsD820ReturnRva);status_text(" ab9830_return=");status_hex32(kTargetAB9830ReturnRva);status_text("\r\n");
    ...
    status_text("ABC510_PATH_GATE ");status_text(abc_ok?"ACTIVE":"NOT_INSTALLED");status_text("\r\nAB9830_EXPERIMENT ");status_text(curve_ok?"ACTIVE":"NOT_INSTALLED");status_text(" bypass=");status_dec((UINT32)g_bypass_enabled);status_text("\r\n");

    LogRecordV6 meta;common_fill(&meta,REC_META,0);meta.c.caller_address=meta.c.caller_module_base=meta.c.caller_rva=0;meta.p.meta.processor_rva=processor_rva;meta.p.meta.signature_matches=matches;meta.p.meta.callsite_count=call_count;meta.p.meta.d820_hook_installed=d820_ok?1U:0U;meta.p.meta.di_iat_patched=(UINT32)g_di_iat_patched;meta.p.meta.raw_monitor_active=1;meta.p.meta.abc510_rva=abc_rva;meta.p.meta.abc510_signature_matches=abc_matches;meta.p.meta.abc510_hook_installed=abc_ok?1U:0U;meta.p.meta.ab9830_rva=curve_rva;meta.p.meta.ab9830_signature_matches=curve_matches;meta.p.meta.ab9830_hook_installed=curve_ok?1U:0U;meta.p.meta.target_abc510_return_rva=kHipABC510ReturnRva;meta.p.meta.target_ab9830_return_rva=kTargetAB9830ReturnRva;meta.p.meta.target_d820_return_rva=kHipD820ReturnRva;meta.p.meta.ads_abc510_return_rva=kAdsABC510ReturnRva;meta.p.meta.ads_d820_return_rva=kAdsD820ReturnRva;meta.p.meta.bypass_enabled=(UINT32)g_bypass_enabled;meta.p.meta.fresh_window_us=kFreshMouseWindowUs;meta.p.meta.tail_timeout_us=kTailTimeoutUs;meta.p.meta.tail_stable_samples=kTailStableSamples;meta.c.qpc_end=qpc_now();enqueue_record(&meta);
    ...
    status_text("EXPECTED: when enabled, AB9830 bypass requires targeted fresh mouse; D820 unclamp requires camera caller + active mouse-tail latch + validated post-state model + preclamp beyond limit.\r\n");return 0;
}
